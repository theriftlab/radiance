...
test('add', function () {
    ...
    expect(fn () => $this->latitude->add($this->decimalAngle))->toThrow(LatitudeBoundaryError::class);
});
...
test('sub', function () {
    ...
    expect(fn () => $this->negativeLatitude->sub($this->decimalAngle))->toThrow(LatitudeBoundaryError::class);
});
...
test('distanceTo', function () {
    ...
    expect(fn () => $this->latitude->distanceTo(91))->toThrow(LatitudeBoundaryError::class);
});
...
test('distanceFrom', function () {
    ...
    expect(fn () => $this->latitude->distanceFrom(-91))->toThrow(LatitudeBoundaryError::class);
});
...
test('midpointWith', function () {
    ...
    expect(fn () => Latitude::make(20)->midpointWith(91))->toThrow(LatitudeBoundaryError::class);
});
